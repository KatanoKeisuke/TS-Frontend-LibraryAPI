...
export class BookRepository implements IBookRepository {
    ...
    public async searchKeyword(keyword: string): Promise<Book[]> {
        ...
        const response = await fetch(`/proxy-api/app2/library/api/books?${params.toString()}`, {
            method: "GET",
            headers: {
    ...
    async existsByName(title: string): Promise<void> {
        const session = await getSession();
        const token = (session as any)?.user?.token;
        const params = new URLSearchParams({ bookTitle: title });
    ...
    async register(book: BookRegistration): Promise<Book> {
        const session = await getSession();
        const token = (session as any)?.user?.token;
        const response = await fetch("/proxy-api/app2/library/api/books", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${token}`,
                "Content-Type": "application/json"
